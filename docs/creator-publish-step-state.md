...
配置管理「自动调度」每次命中时间槽后的顺序：

1. 先扫描本地 `failed` 任务，把可重试失败任务加入 `queued`，避免被后续 AI 正文生成或飞书附件下载阻塞。
2. 扫描飞书任务表，得到本次满足导入条件的候选记录。
3. 仅对这些本次导入候选里「正文」为空、且能匹配商品文案提示词的记录生成 AI 正文并写回飞书。
4. 再执行飞书导入；导入过程中会把新建/更新/可重试的本地任务加入 `queued`。
